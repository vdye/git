...
test_expect_success 'read-tree without merge' '
	init_repos &&

	# Read a tree with no changes outside the sparse cone
	test_all_match git read-tree update-deep &&
	test_all_match git status --porcelain=v2 &&

	# Read a tree with changes outside the sparse cone
	test_sparse_match git read-tree update-folder1 &&
	test_sparse_match git status --porcelain=v2
'
