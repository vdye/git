...
# TODO: pre-dating any sparse index compatibility changes to `read-tree`,
# this merge fails due to a status difference between the sparse index and
# non-sparse index checkouts. Specifically, `folder2/0` (a directory) appears
# as modified in the sparse index, but `folder2/0/a` (correctly?) appears
# instead as a modified entry in the non-sparse index checkouts.
