...
static int sparse_checkout_list(int argc, const char **argv, const char *prefix)
{
	...
	setup_work_tree();
	if (!the_repository->settings.core_apply_sparse_checkout)
...
static int write_patterns_and_update(struct pattern_list *pl)
{
	...
	if (the_repository->settings.core_sparse_checkout_cone)
...
static enum sparse_checkout_mode update_cone_mode(int *cone_mode) {
	...
	if (*cone_mode == -1 && the_repository->settings.core_apply_sparse_checkout)
		*cone_mode = the_repository->settings.core_sparse_checkout_cone;

	/* Set cone/non-cone mode appropriately */
	the_repository->settings.core_apply_sparse_checkout = 1;
	if (*cone_mode == 1 || *cone_mode == -1) {
		the_repository->settings.core_sparse_checkout_cone = 1;
		return MODE_CONE_PATTERNS;
	}
	the_repository->settings.core_sparse_checkout_cone = 0;
...
static int update_modes(int *cone_mode, int *sparse_index)
{
	...
	record_mode = (*cone_mode != -1) || !the_repository->settings.core_apply_sparse_checkout;
				   ...
				   int use_stdin)
{
	...
	existing.use_cone_patterns = the_repository->settings.core_sparse_checkout_cone;
			   ...
			   const char *prefix, int skip_checks)
{
	...
	if (prefix && *prefix && the_repository->settings.core_sparse_checkout_cone) {
		/*
...
static int sparse_checkout_set(int argc, const char **argv, const char *prefix)
{
	 ...
	 */
	if (!the_repository->settings.core_sparse_checkout_cone && argc == 0) {
